...
def shared_rides_not_found(request):
    return render(request, "company_portal/error.html", {"error_message": "404 Page Not Found"})


def shared_rides_server_error(request):
    return render(request, "company_portal/error.html", {"error_message": "500 Internal Server Error"})


def shared_rides_permission_denied(request):
    return render(request, "company_portal/error.html", {"error_message": "403 Forbidden"})


def shared_rides_bad_request(request):
    return render(request, "company_portal/error.html", {"error_message": "400 Bad Request"})
